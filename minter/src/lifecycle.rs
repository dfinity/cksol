...
use crate::state::{State, init_state, mutate_state};
...
pub fn init(init_args: InitArgs) {
    ...
    init_state(State::try_from(init_args.clone()).expect("ERROR: invalid init args"));
...
pub fn post_upgrade(upgrade_args: Option<UpgradeArgs>) {
    ...
    init_state(with_event_iter(|events| replay_events(events)));
