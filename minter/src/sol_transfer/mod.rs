...
/// Maximum number of withdrawal transfers in a single Solana transaction.
/// Withdrawal transactions have only one signer (the minter), so the binding
/// constraint is the serialised transaction size (MAX_TX_SIZE), not the number
/// of signatures. We use the same limit as deposit consolidation for simplicity.
pub const MAX_WITHDRAWALS_PER_TX: usize = MAX_SIGNATURES as usize - 1;
...
pub async fn create_signed_withdrawal_transaction(
    ...
    let instructions: Vec<Instruction> = destinations
        .iter()
        .map(|(destination, amount)| instruction::transfer(&fee_payer_address, destination, *amount))
        .collect();
