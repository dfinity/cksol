...
mod process_pending_withdrawals_tests {
    ...
    async fn should_batch_multiple_withdrawals_into_one_tx() {
        ...
        EventsAssert::from_recorded()
            ...
            .expect_event(|e| {
                assert_matches!(e, EventType::SubmittedTransaction { purpose, .. } => {
                    // All 3 withdrawals share the same transaction
                    assert_matches!(purpose, TransactionPurpose::WithdrawSol { burn_indices } => {
                        assert_eq!(burn_indices, [1u64,2,3].map(LedgerBurnIndex::new));
                    });
        ...
        EventsAssert::from_recorded()
            ...
            .expect_event(|e| {
                assert_matches!(e, EventType::SubmittedTransaction {
                    ...
                    assert_eq!(*burn_indices, [1u64,2].map(LedgerBurnIndex::new));
                });
            })
            .assert_no_more_events();
